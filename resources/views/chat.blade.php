...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased">
                ...
                <div class="ml-4">
                    <div class="text-lg">
                        @if ($message['role'] === 'assistant')
                            <a href="#" class="font-medium text-gray-900">{{ __('ChatGPT') }}</a>
                        @else
                            <a href="#" class="font-medium text-gray-900">{{ __('You') }}</a>
                        @endif
                    </div>
                    <div class="mt-1">
        ...
        <form class="p-4 flex space-x-4 justify-center items-center" action="/" method="post">
            @csrf
            <label for="message">{{ __('Question:') }}</label>
            <input id="message" type="text" name="message" autocomplete="off" class="border rounded-md  p-2 flex-1" />
            <a class="bg-gray-800 text-white p-2 rounded-md" href="/reset">{{ __('Reset Conversation') }}</a>
        </form>
    </body>
</html>
